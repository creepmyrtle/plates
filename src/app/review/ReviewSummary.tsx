...
import type { Review, PlateReviewRating, PlateWithCounts } from '@/lib/types';

interface Props {
  review: Review;
  ratings: PlateReviewRating[];
  plates: PlateWithCounts[];
}

const moodEmojis = ['😫', '😔', '😐', '🙂', '😄'];
const moodLabels = ['Rough', 'Tough', 'Okay', 'Good', 'Great'];

export default function ReviewSummary({ review, ratings, plates }: Props) {
        ...
        {ratings.length > 0 && (
          <div className="rounded-lg border border-border bg-bg-card p-4">
            ...
            <div className="mt-3 space-y-3">
              {ratings.map((r) => {
                const plate = plates.find((p) => p.id === r.plate_id);
                  ...
                  <div key={r.id}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium">{plate.name}</span>
                      </div>
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((v) => (
                          <div
                            ...
                            style={v <= r.rating ? { backgroundColor: plate.color } : undefined}
                          />
                        ))}
                      </div>
                    </div>
